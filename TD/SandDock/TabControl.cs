...
namespace FQ.FreeDock
{
	...
	public class TabControl : Control
	{
		...
		private void xb30ec7cfdf3e5c19(Graphics x41347a961b838962, ITabControlRenderer x38870620fd380a6b, x0a9f5257a10031b2 x128517d7ded59312, SandDockButtonType x271bd5d42b3ea793, bool x2fef7d841879a711)
		{
			...
			if ((uint)x2fef7d841879a711 - (uint)x2fef7d841879a711 <= uint.MaxValue || 4 == 0)
				goto label_1;
			else
				goto label_2;
			label_5:
			...
			if ((uint)x2fef7d841879a711 + (uint)x2fef7d841879a711 >= 0U)
				goto label_7;
			label_12:
		...
		internal void x436f6f3ee14607e0()
		{
			...
			if (1 != 0)
			{
				...
				else if ((uint)num4 - (uint)num4 >= 0U)
					return;
				else
					goto label_29;
			}
			...
			while ((uint)width >= 0U)
			{
				...
				{
					IDisposable disposable = enumerator as IDisposable;
					if ((uint)num4 + (uint)num4 > uint.MaxValue || disposable != null)
						disposable.Dispose();
				}
		...
		private void xad3ea5eacdd3e808()
		{
			...
			finally
			{
				...
				if ((uint)num2 + (uint)width1 < 0U || disposable != null)
					disposable.Dispose();
			}
			label_23:
			...
			bool flag2;
			bool flag3;
			if ((uint)flag2 + (uint)flag3 <= uint.MaxValue)
			{
				...
				foreach (TabPage tabPage in (ArrangedElementCollection) this.Controls)
				{
					...
					while ((uint)left1 - (uint)flag4 > uint.MaxValue);
					left1 = this.xd2fe3b65e7e0ab37.Left;
					arrayList4.Add((object)tabPage);
		...
		private void x9ad45a8b0cdc25f7()
		{
			...
			finally
			{
				...
				{
					disposable.Dispose();
					if ((uint)width + (uint)width >= 0U && 0 == 0)
						break;
				}
			}
		}
		...
		private void x35cf6ce73d51ebeb(int x23e85093ba3a7d1d, bool x17cc8f73454a0462)
		{
			...
			try
			{
				while (enumerator.MoveNext() || 0 != 0 || (num2 | 1) == 0)
				{
